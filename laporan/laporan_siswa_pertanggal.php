...
//GET AND POST TANGGAL MULAI-------------------------------------------------------------
$default_hari = date('d');
if(!empty($_POST["tanggal_mulai"])){
	$tanggal_mulai = $_POST["tanggal_mulai"];
}
else if(!empty($_GET["tanggal_mulai"])){
	$tanggal_mulai = $_GET["tanggal_mulai"];
}
else{
	$tanggal_mulai = "-";
}
$tgl_start = $tanggal_mulai."-".$bulan."-".$tahun;
//GET AND POST TANGGAL AKHIR-------------------------------------------------------------
if(!empty($_POST["tanggal_akhir"])){
	$tanggal_akhir = $_POST["tanggal_akhir"];
}
else if(!empty($_GET["tanggal_akhir"])){
	$tanggal_akhir = $_GET["tanggal_akhir"];
}
else{
	$tanggal_akhir = "-";
}
$tgl_finish = $tanggal_akhir."-".$bulan."-".$tahun;
//----------------------------------------------------------------------------------------
...
<?php //----------------------------------------------------------------------------------------------------------------------------?>				
				...
				<option <?php if($tanggal_mulai == "-"){ print "selected=true"; }?>>Mulai</option>
					<?php	}
					...
					?>
						<option value='<?php print $b;?>' <?php if($tanggal_mulai == $b){ print "selected=true"; }?> ><?php print $b; ?></option>
					<?php } ?>
					</select>
					<select name="tanggal_akhir">	
					<option <?php if($tanggal_akhir == "-"){ print "selected=true"; }?>>Akhir</option>
					<?php	
					...
					?>
						...
						<option value='<?php print $b2;?>' <?php if($tanggal_akhir == $b2){ print "selected='true'"; }?>><?php print $b2; ?></option>
					<?php
					}
	...
	<ul class='download'>
		<li>
				<form method="post" action="laporan/download/download_xl.php">
					<input type="hidden" name="jenis_laporan" value="pertanggal">
					<input type="hidden" name="nis" value="<?php echo  $nis_siswa; ?>">
					<input type="hidden" name="bulan" value="<?php echo  $tanggal_bulan; ?>">
					<input type="hidden" name="mulai" value="<?php echo  $tgl_start; ?>">
					<input type="hidden" name="akhir" value="<?php echo  $tgl_finish; ?>">
...
<!-- ------------------------------------------------------------------------------------------------------------------------------------------------------------ -->
					...
					<?php
						...
						$query = mysql_query("select * from absensi_siswa where no_siswa='$nis_siswa' and tanggal between '$tgl_start' and '$tgl_finish' and bulan='$tanggal_bulan' and in_out='$in_out' ORDER BY tanggal limit $offset, $rec_limit");
						$qc = mysql_query("select count(*) as tot3 from absensi_siswa where no_siswa='$nis_siswa' and tanggal between '$tgl_start' and '$tgl_finish' and bulan='$tanggal_bulan' and in_out='$in_out'");
						$hadir = mysql_query("SELECT * FROM absensi_siswa WHERE no_siswa='$nis_siswa' and keterangan='h' and tanggal between '$tgl_start' and '$tgl_finish' and bulan='$tanggal_bulan' and in_out='$in_out'");
						$alpa = mysql_query("SELECT * FROM absensi_siswa WHERE no_siswa='$nis_siswa' and keterangan='a' and tanggal between '$tgl_start' and '$tgl_finish' and bulan='$tanggal_bulan' and in_out='$in_out'");
						$sakit = mysql_query("SELECT * FROM absensi_siswa WHERE no_siswa='$nis_siswa' and keterangan='s' and tanggal between '$tgl_start' and '$tgl_finish' and bulan='$tanggal_bulan' and in_out='$in_out'");
						$izin = mysql_query("SELECT * FROM absensi_siswa WHERE no_siswa='$nis_siswa' and keterangan='i' and tanggal between '$tgl_start' and '$tgl_finish' and bulan='$tanggal_bulan' and in_out='$in_out'");
						...
						<div class="dataTables_paginate paging_full_numbers">
							<span>
								<?
									...
									for($i=1;$i<=$jumhal;$i++){
								?>
								<a href='index.php?page=absen_siswa_pertanggal&page_laporan=<?php echo $i; ?>&tahun=<?php echo $tahun; ?>&bulan=<?php echo $bulan; ?>&in_out=<?php echo $in_out; ?>&nis_siswa=<?php echo $nis_siswa; ?>&tanggal_mulai=<?php echo $tanggal_mulai; ?>&tanggal_akhir=<?php echo $tanggal_akhir; ?>'><span class="paginate_button"><?=$i?></span></a>
								<?
									}
								?>
							</span>

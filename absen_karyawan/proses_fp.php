...
//	print_r(htmlentities($buffer));
	...
	$query_a=mysql_query("select*from absensi_karyawan where karyawan_nup='$PIN' AND tanggal='$pecah'");
	$isi=mysql_fetch_array($query_a);
	//$tanggal="11-11-11";
	$check_query=mysql_query("select*from karyawan where nup='$PIN'");
	$check_array=mysql_fetch_array($check_query);
	if($check_array['nup'] == $PIN){
		...
		if($nup == 0 || $tanggal == 0){
		...
		else if($nup == $isi['karyawan_nup'] && $tanggal == $isi['tanggal']){
			echo "&nbsp;";
		}
		else if( $isi['karyawan_nup'] != $nup || $isi['tanggal'] != $tanggal){
			...
			mysql_query("insert into absensi_karyawan(karyawan_nup,keterangan,tanggal,bulan,selesai,waktu,terlambat) values('$PIN','h','$pecah','$bulan','yes','$pecah2','$telat')");
		}
		else{
			echo "&nbsp;";
		}	
